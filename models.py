...
class TransE(KBCModel):
    def __init__(self, sizes, rank, init_size):
        ...
        self.relation = nn.Embedding(sizes[1], rank)
...
class UniBi_2(KBCModel):
    ...
    def score(self, x):
        ...
        uh = self.givens_rotations(Rot_u, lhs)
        suh = rel * uh
        vh = self.givens_rotations(Rot_v, suh)
    ...
    def forward(self, x, score_rhs=True, score_rel=False, score_lhs=False):
        ...
        if score_lhs:
            to_score_lhs = self.embeddings[0].weight
            vt = self.givens_rotations(Rot_v, rhs, transpose=True)
            suh = rel * vt
            uh = self.givens_rotations(Rot_u, suh, transpose=True)
...
class CP(KBCModel):
    def __init__(self, sizes, rank, init_size):
        ...
        self.sizes = sizes
        self.rank = rank
        self.lhs = nn.Embedding(sizes[0], rank)
    ...
    def get_candidates(self, chunk_begin, chunk_size, target='rhs', indices=None):
        if target == 'rhs':
            if indices is None:
                ...
                num_cands = indices.shape[1]
                indices = indices[:, num_cands//2:]
                return self.rhs.weight.data[indices.reshape(-1)].reshape(bsz, num_cands//2, -1)
        elif target == 'lhs':
            if indices is None:
                ...
                num_cands = indices.shape[1]
                indices = indices[:, :num_cands//2]
                return self.lhs.weight.data[indices.reshape(-1)].reshape(bsz, num_cands//2, -1)
        elif target == 'rel':
...
class RESCAL(KBCModel):
    def __init__(
        ...
        self.entity = nn.Embedding(sizes[0], rank)
        self.relation = nn.Embedding(sizes[1], rank * rank)
